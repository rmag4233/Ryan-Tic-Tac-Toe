...
var updateGame = function updateGame(id, text, over) {
  return $.ajax({
...
var showOneGame = function showOneGame(data) {
  return $.ajax({
    method: 'GET',
...
var onAddSuccess = function onAddSuccess(games) {
...
var onAddGame = function onAddGame(event) {
  ...
  api.startGame().then(ui.onAddSuccess).catch(ui.onError);
};
...
var onGetOneGame = function onGetOneGame(event) {
  ...
  if (data.game.id.length !== 0) {
    api.showOneGame(data).then(onGetOneGameSuccess).catch(ui.onError);
  } else {}$('#game-id').val('');
  $('#contentAdd').text('');
  $('#getGame').text('Please enter in a game ID.');
};

var onGetOneGameSuccess = function onGetOneGameSuccess(data) {
...
var onGetGames = function onGetGames(event) {
  api.getGames().then(ui.getGamesSuccess).catch(ui.onError).then(function (games) {});
};
